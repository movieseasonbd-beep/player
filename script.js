...
// পারফরম্যান্স উন্নতি: Throttling এর জন্য ভ্যারিয়েবল
...
// ==========================================================
// === Functions (কিছু ফাংশন পারফরম্যান্সের জন্য উন্নত করা হয়েছে) ===
// ==========================================================

const acquireWakeLock = async () => { if ('wakeLock' in navigator) { try { wakeLock = await navigator.wakeLock.request('screen'); } catch (err) { console.error(`Wake Lock request failed: ${err.name}, ${err.message}`); } } };
const releaseWakeLock = () => { if (wakeLock !== null) { wakeLock.release(); wakeLock = null; } };
function initializeHls() { if (hls) { hls.destroy(); } hls = new Hls(hlsConfig); addHlsEvents(); }

function loadVideo(videoUrl) {
    setTimeout(() => { if (!loadingOverlay.classList.contains('hidden')) { loadingOverlay.classList.add('hidden'); } }, 3000);
    if (Hls.isSupported() && videoUrl.includes('.m3u8')) { initializeHls(); hls.loadSource(videoUrl); hls.attachMedia(video); } else { video.src = videoUrl; }
}
...
function setQuality(level, url = null) {
    ...
    if (url) {
        ...
        hls.once(Hls.Events.LEVEL_LOADED, () => { video.currentTime = currentTime; if (isPlaying) { video.play(); } });
...
function handleScreenTap() {
    ...
    if (video.paused) { video.play(); } else {
        if (isControlsVisible) { video.pause(); } else { showControlsAndResetTimer(); }
    }
}
...
// পারফরম্যান্স উন্নতি: এই ফাংশনটি এখন debounced handler দ্বারা কল হবে
function showControlsAndResetTimer() { playerContainer.classList.add('show-controls'); resetControlsTimer(); }

// পারফরম্যান্স উন্নতি: প্রোগ্রেস বার এখন কম আপডেট হবে (Throttled)
function updateProgressUI(e) {
    if (isScrubbing) return;
    const now = performance.now();
    // প্রতি 250ms-এ একবারের বেশি আপডেট হবে না
    if (now - lastTimeUpdate < 250 && e.type !== 'canplay') return; 
    lastTimeUpdate = now;
...
// পারফরম্যান্স উন্নতি: স্ক্রাবিং UI এবং ভিডিও Seek আলাদা করা হয়েছে
function updateScrubUI(e) {
...
function showMenuPage(pageToShow) {
    ...
    setTimeout(() => { const newHeight = pageToShow.scrollHeight; menuContentWrapper.style.height = `${newHeight}px`; }, 0);
    if (currentPage) {
        if (pageToShow === mainSettingsPage) {
            currentPage.classList.remove('active'); currentPage.classList.add('slide-out-right');
            mainSettingsPage.classList.remove('slide-out-left'); mainSettingsPage.classList.add('active');
        } else {
            mainSettingsPage.classList.add('slide-out-left'); currentPage.classList.remove('active');
            pageToShow.classList.add('active'); pageToShow.classList.remove('slide-out-right');
        }
    }
}

// পারফরম্যান্স উন্নতি: Debounce ফাংশন
...
// পারফরম্যান্স উন্নতি: স্ক্রাবিং ইভেন্টগুলো অপটিমাইজ করা হয়েছে
progressBar.addEventListener('input', updateScrubUI); // শুধু UI আপডেট
progressBar.addEventListener('change', setScrubTime); // মাউস ছাড়ার পর ভিডিও Seek
...
// পারফরম্যান্স উন্নতি: mousemove এখন debounced
const debouncedShowControls = debounce(showControlsAndResetTimer, 50);
playerContainer.addEventListener('mousemove', debouncedShowControls);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (videoUrl) { loadVideo(videoUrl); } else { loadingOverlay.classList.remove('hidden'); loadingOverlay.querySelector('.loading-text').textContent = "No video source found."; }
    updatePlayState();
    updateVolumeIcon();
